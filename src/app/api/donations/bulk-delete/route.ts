 ...
 * Delete multiple donations in bulk
 */
export const POST = buildApiRoute({
  ...
  logger.info('Bulk delete donations', { count: ids.length, ids: ids.slice(0, 5) }); // Log first 5 IDs for debugging

  // Delete all donations in parallel
  const deleteResults = await Promise.allSettled(
    ids.map(id => appwriteDonations.remove(id))
  );
  ...
  deleteResults.forEach((result, index) => {
    ...
    if (result.status === 'fulfilled') {
      ...
      logger.error('Failed to delete donation', { donationId: id, error: result.reason });
    }
  });
  ...
  }, `${deleted.length} bağış başarıyla silindi${failed.length > 0 ? `, ${failed.length} bağış silinemedi` : ''}`);
});
