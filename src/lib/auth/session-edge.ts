 ...
 */
export async function parseAuthSessionEdge(cookieValue?: string): Promise<AuthSession | null> {
  if (!cookieValue) {
    return null;
  }
  ...
  if (payload && signature) {
    try {
      ...
      const jsonString = atob(payload.replace(/-/g, '+').replace(/_/g, '/'));
 ...
 */
export async function getAuthSessionFromRequestEdge(request: NextRequest): Promise<AuthSession | null> {
