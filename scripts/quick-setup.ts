...
if (existsSync(envLocalPath)) {
  console.log('ℹ️  .env.local dosyası bulundu, mevcut değerler korunacak...');
  envContent = readFileSync(envLocalPath, 'utf-8');
} else {
  console.log('ℹ️  .env.local dosyası oluşturuluyor...');
}
...
// Her değişkeni kontrol et, yoksa ekle
let addedCount = 0;
for (const [key, value] of Object.entries(envVars)) {
  const regex = new RegExp(`^${key}=.*$`, 'm');
  if (!regex.test(envContent)) {
    // Eğer değişken dosyada yoksa ekle
...
if (addedCount > 0) {
  console.log(`✅ .env.local güncellendi (${addedCount} yeni değişken eklendi)`);
} else {
  console.log('✅ .env.local zaten güncel');
}
